...
{ 
	...
	Write-Host "It is in InstallPreReqFiles()"
     
        Try 
        { 
        Write-Host "In TryBloc of InstallPreReqFiles()"
	...
	Write-Host "Try finish"
        } 
        Catch 
        { 
	Write-Host "CatchBlock"
            $ReturnCode = -1 
            Write-Error $_ 
            break 
        }     
...
{ 
Write-Host "Entering into InstallPreReqs()"
    $rc = 0 
